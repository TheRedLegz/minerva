...
class DatabaseConnection:
    ...
    def get_raw_tweets(self):
        ...
        data = []
        raw_tweet_collection = list(
            self.client[self.db_name]['rawtweets'].find())

        for tweet in raw_tweet_collection:
            data.append(
                {'tweet_id': tweet['tweet_id'], 'full_text': tweet['data']['full_text']})
    ...
    def get_preprocessed_tweets(self):
        ...
        data = []
        raw_tweet_collection = list(
            self.client[self.db_name]['preprocessed_tweets'].find())

        for tweet in raw_tweet_collection:
            data.append(
                {'tweet_id': tweet['tweet_id'], 'preprocessed_text': tweet['preprocessed_text']})
    ...
    def get_unprocessed_tweets(self):
        ...
        preprocessed_tweet_ids = [tweet['tweet_id']
                                  for tweet in preprocessed_tweet_collection]
        unprocessed_tweets = list(db['rawtweets'].find(
            {'tweet_id': {'$nin': preprocessed_tweet_ids}}))
    ...
    def get_tweet_text_by_id_array(self, tweet_id_array):
        tweet_list = list(self.client[self.db_name]['rawtweets'].find(
            {'tweet_id': {'$in': tweet_id_array}}))
        tweet_list = [tweet['data']['full_text'] for tweet in tweet_list]
        return tweet_list

    def get_tweet_text_by_id(self, tweet_id):
        tweet = list(self.client[self.db_name]
                     ['rawtweets'].find({'tweet_id': tweet_id}))
